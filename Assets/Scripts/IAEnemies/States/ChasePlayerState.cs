...
public class ChasePlayerState : EnemyState
{
    ...
    {
        _chaseOffset = new Vector2(chaseOffset.x * -1 * Mathf.Sign(direction.x), chaseOffset.y * -1 * Mathf.Sign(direction.y))*-1;
        if (canFly)
        {
    ...
    public override void OnUpdateState()
    {
        ...
        direction = (Player.Instance.transform.position - this.transform.position + (Vector3)_chaseOffset).normalized;
        distanceToTarget = Vector2.Distance(Player.Instance.transform.position + (Vector3)_chaseOffset, this.transform.position);
